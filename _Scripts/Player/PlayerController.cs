...
namespace UnityTutorial.PlayerControl
{
    public class PlayerController : MonoBehaviour
    {
        ...
        private void Start() {
            defaultCameraPosition = Camera.localPosition;
            defaultCameraRotation = Camera.localRotation;
        ...
        private bool zKeyHeld = false;
        private float zKeyStartTime;
        ...
        IEnumerator ZoomOut()
        {
            ...
            {
                Camera.localPosition = Vector3.Lerp(defaultCameraPosition, new Vector3(0f, 1.5f, -3f), elapsed_time / duration);
                Camera.localRotation = Quaternion.Slerp(defaultCameraRotation, Quaternion.Euler(15f, 0f, 0f), elapsed_time / duration);

                elapsed_time += Time.deltaTime;
                yield return null;
            }

            // Optionally, add a delay here if you want the camera to stay in the zoomed-out position for some time
        ...
        IEnumerator ZoomIn()
        {
            ...
            while (elapsed_time < duration)
            {
                Camera.localPosition = Vector3.Lerp(Camera.localPosition, defaultCameraPosition, elapsed_time / duration);
                Camera.localRotation = Quaternion.Slerp(Camera.localRotation, defaultCameraRotation, elapsed_time / duration);
